...
for initial_pool_size in initial_pool_sizes:
    ...
    for iteration in range(num_iterations):
        ...
        new_samples = query_strategy(model, mnist_unlabeled)
        new_sample_indices = get_indices_from_subset(new_samples)

        initial_pool_indices, unlabeled_pool_indices = update_pools(initial_pool_indices, unlabeled_pool_indices,
                                                                    new_sample_indices)
...
with open('./file_path_uncertain', "w") as json_file:
